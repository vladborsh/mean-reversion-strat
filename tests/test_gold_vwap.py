...
This script validates the VWAP detector by:
1. Fetching historical GOLD data from Capital.com for a specified date range
2. Running the detector on each eligible candle (13:00-15:00 UTC)
...
class GoldVWAPTester:
    """
    Test harness for GOLD VWAP Mean Reversion strategy.
    """
    ...
    def __init__(self, strategy_config: dict):
        ...
        self.detector = VWAPDetector(
            num_std=strategy_config.get('num_std', 1.0),
            signal_window_start=strategy_config.get('signal_window_start', '13:00'),
            signal_window_end=strategy_config.get('signal_window_end', '15:00'),
            anchor_period=strategy_config.get('anchor_period', 'day')
        )
    ...
    def run_backtest(self, data: pd.DataFrame, symbol: str = 'GOLD') -> List[Dict]:
        """
        ...
        # We need at least enough data to cover one full day for VWAP calculation
        ...
        for i in range(min_required, len(data)):
            ...
            # Only analyze during signal window (13:00-15:00 UTC)
            ...
            try:
                ...
                logger.error(f"Error detecting signal at {current_timestamp}: {e}")
                continue
    ...
    def generate_report(self) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append("GOLD VWAP MEAN REVERSION STRATEGY - BACKTEST REPORT")
        report.append("=" * 80)
        report.append("")
        ...
        if long_signals:
            ...
            for idx, sig in enumerate(long_signals, 1):
                ...
                report.append(f"   VWAP:           {sig.get('vwap', 'N/A')}")
                report.append(f"   VWAP Upper:     {sig.get('vwap_upper', 'N/A')}")
                report.append(f"   VWAP Lower:     {sig.get('vwap_lower', 'N/A')}")
    ...
    def export_signals_csv(self, output_path: str):
        ...
        columns = [
            ...
            'current_price', 'vwap', 'vwap_upper', 'vwap_lower', 'reason'
        ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Test GOLD VWAP Mean Reversion Strategy',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Test for December 2025 (using default config)
  python tests/test_gold_vwap.py --start 2025-12-01 --end 2025-12-24 --symbol GOLD
  
  # Test with custom config file
  python tests/test_gold_vwap.py --start 2025-12-01 --end 2025-12-24 --symbol GOLD --config my_config.json
  
  # Export signals to CSV
  python tests/test_gold_vwap.py --start 2025-12-01 --end 2025-12-24 --symbol GOLD --export signals.csv
        """
    )
    ...
    parser.add_argument('--symbol', default='GOLD', help='Asset symbol (default: GOLD)')
    ...
    try:
        ...
        logger.info(f"VWAP std dev: {strategy_params.get('num_std')}")
        logger.info(f"Signal window: {strategy_params.get('signal_window_start')}-{strategy_params.get('signal_window_end')}")
        logger.info(f"Anchor period: {strategy_params.get('anchor_period')}")
    except Exception as e:
    ...
    # Initialize tester
    try:
        tester = GoldVWAPTester(strategy_config=strategy_params)
    ...
    # Run backtest
    signals = tester.run_backtest(data, symbol=args.symbol)
